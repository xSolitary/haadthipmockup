            ...
            <DataTable headers={["PO", "ผู้ขาย", "ยอดใบแจ้งหนี้", "สถานะ"]}>
              {paymentRequests.map((payment) => (
                <tr key={payment.id} className="cursor-pointer border-t border-slate-100 hover:bg-slate-50" onClick={() => setSelectedPaymentId(payment.id)}>
                  <td className="px-4 py-4">{payment.poNumber}</td>
                  <td className="px-4 py-4">{payment.vendorName}</td>
                  <td className="px-4 py-4">{formatCurrency(payment.invoiceAmount)}</td>
                  <td className="px-4 py-4"><StatusBadge label={payment.status} /></td>
                </tr>
              ))}
            </DataTable>
          </div>
            ...
            {selectedPayment ? (
              <>
                ...
                <div className="grid gap-4 text-sm text-slate-600">
                  ...
                  <label className="rounded-3xl bg-slate-100 p-4 text-sm text-slate-700">
                  ...
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center rounded-2xl bg-[#007946] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#005f37] disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500"
                  >
                    {actionLabel}
                  </button>
                </div>
