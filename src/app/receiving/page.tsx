            ...
            {waitingOrders.length === 0 ? (
              ...
              waitingOrders.map((po) => (
                <button
                  ...
                  className={`w-full rounded-3xl border px-4 py-4 text-left transition ${selectedPoId === po.id ? "border-sky-400 bg-sky-50" : "border-slate-200 bg-white hover:bg-slate-50"}`}
                >
              ...
              <>
                ...
                <div className="grid gap-4 text-sm text-slate-600">
                  <label className="space-y-2 text-slate-700">
                    วันที่ส่งมอบ
                    <input type="date" value={deliveryDate} onChange={(event) => setDeliveryDate(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" />
                  </label>
                  <label className="space-y-2 text-slate-700">
                    จำนวนที่รับ
                    <input type="number" min={0} value={receivedQty} onChange={(event) => setReceivedQty(Number(event.target.value))} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" />
                  </label>
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="space-y-2 text-slate-700">
                      สภาพสินค้า
                      <select value={condition} onChange={(event) => setCondition(event.target.value as typeof condition)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900">
                        <option>Good</option>
                    ...
                    <label className="space-y-2 text-slate-700">
                      QC Required
                      <select value={qcRequired ? "yes" : "no"} onChange={(event) => setQcRequired(event.target.value === "yes")} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="space-y-2 text-slate-700">
                      เลขที่ LOT
                      <input value={lotNumber} onChange={(event) => setLotNumber(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" />
                    </label>
                    <label className="space-y-2 text-slate-700">
                      เลขที่ Batch
                      <input value={batchNumber} onChange={(event) => setBatchNumber(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" />
                    </label>
                  </div>
                  <label className="space-y-2 text-slate-700">
                    วันหมดอายุ
                    <input type="date" value={expiryDate} onChange={(event) => setExpiryDate(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" />
                  </label>
                  ...
                  <button type="button" onClick={handleReceive} className="inline-flex items-center justify-center rounded-2xl bg-[#007946] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#005f37]">
                    รับสินค้า
                  </button>
                </div>
              </>
          ...
          {existingRecord ? (
            <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
              <div className="flex items-center justify-between gap-3">
                ...
                <button type="button" onClick={() => selectedOrder && markQcPassed(selectedOrder.id)} className="rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">
                  Mark QC Passed
                </button>
              </div>
            </div>
